...
int main() {
    ...
    {
        OrderBook book(200000);                          // fresh book
        const uint32_t NUM_ORDERS = 1'000'000;
        for (uint32_t i = 0; i < NUM_ORDERS; i++) {
            book.add_order(i+1, prices[i], quantities[i], sides[i]);  // pre-fill
        }
        uint64_t start = now_ns();
        for (uint32_t i = 0; i < NUM_ORDERS; i++) {
            book.execute_order(i+1, quantities[i]);      // now orders exist
        }
