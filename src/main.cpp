...
int main() {
    ...
    // ─── execute_order() tests ────────────────────────────────────────
    {
        ...
        std::cout << "[T15] order 4001 qty: " << book.bids[15000]->head->quantity
                << " (expected 300)" << std::endl;
        std::cout << "[T15] level volume:   " << book.bids[15000]->total_volume
                << " (expected 600)" << std::endl;

        // Test 16 — full fill removes order
        book.execute_order(4001, 300);
        std::cout << "[T16] new head qty:   " << book.bids[15000]->head->quantity
                << " (expected 300)" << std::endl;
        ...
        std::cout << "[T17] bids[15000] null: " << (book.bids[15000] == nullptr)
                << " (expected 1)" << std::endl;

        // Test 18 — overfill guard
        book.execute_order(4003, 999);
        std::cout << "[T18] order 4003 still exists: " << (book.order_map.find(4003) != book.order_map.end())
                << " (expected 1)" << std::endl;
